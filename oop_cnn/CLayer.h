...
public:
	Layer_ReLU(string _name, int _fK, int _fC_in, int _fC_out) : Layer::Layer(_name, _fK, _fC_in, _fC_out) // 초기화
	{
		// (구현할 것)
...
public:
	...
	void init(int init_type) {
		...
		else if (init_type == LOAD_INIT) { // 로드 이닛
			for (int w = 0; w < fC_out; w++) {
				for (int z = 0; z < fC_in; z++) {
					for (int x = 0; x < fK; x++) {
						for (int y = 0; y < fK; y++) {
				...
				for (int w = offset; w < nW - offset; w++) {
					...
					for (int ph = 0; ph < fK; ph++) {
						for (int pw = 0; pw < fK; pw++) {
							for (y = 0; y < nC; y++) { // 채널 먼저 돌리기
								sum += (input->get_elem(ph + h - offset, pw + w - offset, y) * weight_tensor[ph][pw][y][x]);
							}
						}
					}
					output->set_elem(h, w, x, sum + bias_tensor[x]); // 바이어스 더하기
				}
			}
		}
